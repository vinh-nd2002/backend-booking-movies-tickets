...
import com.hust.config.GoogleDriveConfig;
...
public class GoogleFileManager {
	...
	public List<File> listEverything() throws IOException, GeneralSecurityException {
		...
		FileList result = googleDriveConfig.getInstance().files().list().setPageSize(1000)
				.setFields("nextPageToken, files(id, name, size, thumbnailLink, shared)") // get field of google drive
																							// file
				.execute();
	...
	// Get file id
	public File getFileDetail(String fileId) throws IOException, GeneralSecurityException {
	...
	public List<File> listFolderContent(String parentId) throws IOException, GeneralSecurityException {
		...
		FileList result = googleDriveConfig.getInstance().files().list().setQ(query).setPageSize(10)
	...
	public String uploadFile(MultipartFile file, String folderName, String type, String role) {
		try {
			...
			if (null != file) {
				...
				File uploadFile = googleDriveConfig.getInstance().files().create(fileMetadata,
				...
				if (!type.equals("private") && !role.equals("private")) {
					...
					googleDriveConfig.getInstance().permissions().create(uploadFile.getId(), setPermission(type, role))
							.execute();
				}
